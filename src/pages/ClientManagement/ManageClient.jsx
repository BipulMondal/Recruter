...
const ManageSubCategory = () => {
    ...
    if (result) {
      let arr = result?.data.map((client, index) => {
        return {
          ...
          // ),
          Email: <div style={{ fontSize: "13px" }}>{client?.email}</div>,
          Mobile: <div style={{ fontSize: "13px" }}>{client?.mobile}</div>,
          Location: (
  ...
  const columns = [
    ...
    {
      name: (
        <div style={{ fontSize: "14px", fontWeight: "bolder" }}>Location</div>
      ),
